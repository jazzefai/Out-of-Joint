          ...
          {(["A", "B", "C"] as const).map((opt) => {
            ...
            return (
              <button
                ...
                className={`w-full py-7 border-2 transition-all duration-150 active:scale-[1.02] ${
                  isSelected
                    ? "border-accent bg-accent text-white vote-selected"
                    : "border-[#2A2A2A] hover:border-accent/60 text-accent"
                }`}
              >
                <span className="font-display text-4xl tabular-nums">{opt}</span>
              </button>
            );
          })}
        </div>
